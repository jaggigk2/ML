...
def OOB_Error(X_test, X_train, instance, oob_errors, y_train):
    # for OOB error using built in random forest
    clf_sk = RandomForestClassifier(n_estimators=10, max_features=instance, oob_score=True)
    clf_sk.fit(X_train, y_train)
    y_pred_sk = clf_sk.predict(X_test)
    oob_error = 1 - clf_sk.oob_score_
    print("OOB_error = %.3f when max_features = %.1f\n" % (oob_error, instance))
...
def run_random_forest():
    ...
    for instance in max_features_array:
        ...
        print("Sensitivity = %.3f when max_features = %.1f" % (sensitivity,instance))

        OOB_Error(X_test, X_train, instance, oob_errors, y_train)

    plt.plot(max_features_array, np.array(oob_errors))
    plt.show()
